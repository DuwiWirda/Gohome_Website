...
@section('container')
    ...
    <main id="main" class="main">
        ...
        <div class="pagetitle" style="display: flex; align-items: center; justify-content: space-between;">
            ...
            <a href="{{ route('pengunjung.refresh') }}" class="btn btn-success button p-2" style="margin-left: 10px;"><i
                    class="bi bi-arrow-clockwise"></i></a>
            <a href="{{ route('pengunjung.add') }}" class="btn btn-success button p-2" style="margin-left: 10px;"><i
                        ...
                        <tbody>
                            @foreach ($pengunjung as $pengunjung)
                                <tr>
                                    ...
                                    <td>
                                        <a href="{{ route('pengunjung.edit', ['nik' => $pengunjung->nik]) }}"
                                            class="btn btn-success button p-2"><i class="bi bi-pencil"></i></a>
                                    </td>
                                    <td>
                                        <form action="{{ route('pengunjung.destroy', ['nik' => $pengunjung->nik]) }}"
                                            method="POST" style="display: inline;">
                                            @csrf
                                            @method('DELETE')
                                            <button type="delete" class="btn btn-success button p-2"
                                                onclick="showSuccessMessage();"><i class="bi bi-trash-fill"></i></button>
                                        </form>
                                    </td>
                                </tr>
                            @endforeach
